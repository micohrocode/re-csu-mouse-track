...
l_b=np.array([29   ,100,20])# lower hsv bound for orange
u_b=np.array([31,255,255])# upper hsv bound to orange
...
while True:
    ...
    if status == "still" and (datetime.now()-click_timer).seconds >= 1:
        pyautogui.click()
        click_timer = None
